...
11.11.2018 JA5AEA: add code for RX1 and RX2 S-ATT control via PE4302 (SPI) and TS-680S CAT control via ttyUSB0.
12.12.2018 JA5AEA: add argumentfor Kenwood CAT parameter selection. Changed from "sdr-transceiver-hpsdr 1 2 2 1 2" 
to "sdr-transceiver-hpsdr 1 2 2 1 2 1". The last argument "1" is for old Kenwood rigs such as TS-680S and TS-690S. "2" is 
for 38400bit 8bit 1stop bit for new generation Kenwood rigs.
21.02.2019 JA5AEA: add code for Audio Codec to run at 12.000MHz clock
23.02.2019 JA5AEA: add code for Audio Codec LINE-IN
*/
...
int main(int argc, char *argv[])
{
  ...
  {
    errno = 0;
    number = (argc == 8) ? strtol(argv[i + 1], &end, 10) : -1;
    if(errno != 0 || end == argv[i + 1] || number < 1 || number > 2)
    {
      printf("Usage: sdr-transceiver-hpsdr 1|2 1|2 1|2 1|2 1|2 1|2 1|2\n");
      return EXIT_FAILURE;
    }
    chan |= (number - 1) << i;
  }
...
void process_ep2(uint8_t *frame)
{
  ...
  uint16_t data;
  uint32_t data32;
  uint8_t ptt, preamp, att, boost, select, linein, linevol; /* select added by JA5AEA */
  uint8_t data8;
...
inline void cw_ptt_off()
{
  ...
  /* PTT off */
  *tx_rst &= ~16;
  if (usb_serial >=0) write(usb_serial,"RX;",3);/* added by JA5AEA */
  cw_ptt = 0;
  /* reset tx fifo */
  *tx_rst |= 1;
  *tx_rst &= ~1;
